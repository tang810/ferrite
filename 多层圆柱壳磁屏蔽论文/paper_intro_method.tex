...
\begin{equation}
    ...
    SF, \qquad
    \mathrm{LeakageRatio}, \qquad
    V_{\mathrm{material}} .
\end{equation}
其中 \(B_{\mathrm{avg},1-30}\) 越低表示材料磁噪声越小，\(SF\) 越高表示外部磁场衰减越强，\(\mathrm{LeakageRatio}\) 越低表示中心泄漏场越小，\(V_{\mathrm{material}}\) 越小表示材料用量越低。后处理时可对各指标归一化，形成多目标评分，用于筛选低噪声、高屏蔽且材料用量较低的候选结构。
