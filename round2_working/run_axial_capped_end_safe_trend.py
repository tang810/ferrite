# -*- coding: utf-8 -*-
"""
Safe trend axial capped-end AEDT experiment for the four-layer ferrite shield.

Run inside Ansys Electronics Desktop:

    Tools -> Run Script -> round2_working/run_axial_capped_end_safe_trend.py

Quick case:
  1. ZCAP_C2_N4_two_caps_no_hole_safe_trend
     Both ends are capped without a cable hole. The cap is intentionally
     thickened for a robust first-pass trend solve.
...
It skips IntH2 and uses a safer simplified cap geometry to get the axial
shielding trend before returning to the thin, holed engineering cap.
...
OUT_CSV = os.path.join(BASE_DIR, "data", "raw", "axial_capped_end_safe_trend_exports.csv")
LOG_PATH = os.path.join(OUT_DIR, "run_axial_capped_end_safe_trend.log")
...
OUTER_R_MM = RIN_MM + FERRITE_TSPACE_MM - 0.10
CAP_T_MM = 0.60
WIRE_HOLE_R_MM = 2.0
CAP_MESH_MAX_LENGTH_MM = 0.30
...
CASES = [
    {
        "case_id": "ZCAP_C2_N4_two_caps_no_hole_safe_trend",
        "top_cap": True,
        "bottom_cap": True,
        "bottom_hole_r_mm": 0.0,
        "notes": "safe trend SFz only; two-end-capped no-hole; cap t=0.60 mm, radius slightly reduced; IntH2 skipped",
    },
]
...
def assign_cap_mesh(fp, oDesign, cap_objects):
    ...
    for obj_name in FERRITE_OBJECTS:
        for prefix in ["Length_surf_z_", "Length_vol_z_", "Length_surf_", "Length_vol_"]:
            try:
                mesh.DeleteMeshOperations([prefix + obj_name])
            except Exception:
                pass
        safe(fp, "SURF safe " + obj_name,
             lambda n=obj_name: mesh.AssignLengthOp([
                 ...
                 "NumMaxElem:=", "4000",
                 "RestrictLength:=", True,
                 "MaxLength:=", str(FERRITE_MESH_MAX_LENGTH_MM) + "mm",
             ]))
        safe(fp, "VOL safe " + obj_name,
             lambda n=obj_name: mesh.AssignLengthOp([
    ...
    if cap_objects:
        try:
            mesh.DeleteMeshOperations(["Length_safe_caps"])
        except Exception:
            pass
        safe(fp, "assign safe cap mesh",
             lambda: mesh.AssignLengthOp([
                 "NAME:Length_safe_caps",
                 "RefineInside:=", True,
                 "Objects:=", cap_objects,
                 "RestrictElem:=", True,
                 "NumMaxElem:=", "4000",
    ...
    log(fp, "safe mesh assigned: ferrite MaxLength=" +
        str(FERRITE_MESH_MAX_LENGTH_MM) + "mm cap MaxLength=" +
...
def process_case(fp, oDesktop, case_info):
    ...
    int_caps = ""
    int_total = ""
    log(fp, "IntH2 skipped in safe trend mode")

    status = "exported_safe_trend" if bz not in ["", None] else "failed"
    out = {
        "case_id": case_id,
        "base_case": BASE_CASE_ID,
        "field_dir": "z",
...
def run():
    ...
    fp = open(LOG_PATH, "wb")
    try:
        log(fp, "run_axial_capped_end_safe_trend " + time.strftime("%Y-%m-%d %H:%M:%S"))
